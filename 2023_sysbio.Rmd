...
.right-plot[
```{r}
arning <- read_csv("data/arning_shuffle.csv")
ggplot(arning) +
...
- This associates new levels with the second source: changing the order of the outcome class then changes this association.
...
- Inserting them there gets rid of the bias towards the second source.
...
pco_cap <- read_csv("data/forest/cap_pco.csv") |>
  mutate(prediction = as_factor(prediction))
...
#  facet_wrap(vars(uses_unique)) +
  scale_y_continuous(limits=c(0,1)) +
  scale_colour_manual(values=c(CA="black", PCO="darkred")) +
  labs(x=NULL, y="Precision") +
  guides(col='none')
```
...
```{r}
read_csv("data/forest/attribution.csv") |>
  mutate(prediction = as_factor(prediction)) |>
  ...
  ggplot() + 
  geom_point(aes(x=prediction, y=prop, col=uses_unique)) +
