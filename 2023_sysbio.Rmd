...
- We have 1342 core genes available to us.
...
## The first problem: High cardinality categorical predictors.
...
- For 2 sources we can do it much faster ($K$ rather than $2^K$) via **target encoding**. Replace the categorical variable with the proportion of training isolates that are the first source in each allele. We then have a numeric variable so need only consider $K-1$ possible split points.

<TODO: Add demo of this>

---

## The second problem: Unique alleles
...
- **If we use an 'encoding' scheme to deal with the high cardinality problem, how should we encode things we haven't yet seen?**
...
.left-code[## Numeric encoding: Consequences
...
- Researchers do not submit isolates randomly to PubMLST. They're often done in 'chunks' of isolates
...
ggplot(camp0864) +
  geom_point(aes(x=Allele, y=Source, col=Source)) +
  guides(col='none') +
  labs(y=NULL, title="CAMP0864 submissions to PubMLST")
```
]
...
Results from their work are rather less impressive when we randomise the
allele labels.
